...
(sqrt(2)) Voigt order, SI units. We reconstruct the full triaxial <D>, take its eigenvalues, and use
the toolbox's principal eigenvector u, the model S0 (the dMRI->T1 registration driver), and MD / uFA.
...
Outputs (registration/, dMRI space):
  ...
  uFA_dMRI.nii.gz              microscopic FA (post-hoc MRE comparison only; see note)
...
# FA(<D>) from the post-fallback eigenvalues — a registration QC + bake-off scoring map (the dMRI->T1
# driver is the model S0, below). Degenerate voxels were isotropized above, so their FA is 0 (not a
# spurious 1).
