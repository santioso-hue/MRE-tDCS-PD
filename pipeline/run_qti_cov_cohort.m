...
%
% Cohort input (MUDI_synb0/<subj>/) is the two b-tensor encodings as SEPARATE Synb0+topup+eddy-corrected,
% UN-smoothed series with bval/bvec (not pre-combined/pre-smoothed), so this fit builds xps+merge+smoothing:
...
% Order-alignment guard: LTE block first, STE block second. b=0 volumes get b_delta=0 by md-dmri
% convention (zero b-tensor has no anisotropy), so require linear only on the b>0 LTE volumes.
...
s     = mdm_mec_eb(s, s_ref, p_fn, op, opt_mec);            % -> LTE_STE_mc.nii.gz

% PD-paper covariance fit, steps replicated explicitly to keep our output names
s = mdm_s_smooth(s, opt.filter_sigma, op, opt);
s = mdm_s_mask(s, @mio_mask_threshold, op, opt);
